...
class TestPrediction(unittest.TestCase):
    ...
    def test_prepare_features(self):
        ...
        # Create mock data as list of dicts (replaces pd.DataFrame)
    ...
    def test_prepare_features_empty(self):
        ...
        X, y_fg, y_time = prepare_features([])
        self.assertEqual(len(X), 0)
        self.assertEqual(len(y_fg), 0)
        self.assertEqual(len(y_time), 0)

    def test_prepare_features_skips_nan(self):
        from app.ml.prediction import prepare_features

        data = [{"timestamp": datetime.now(timezone.utc), "temp": 20.0, "sg": 1.050,
                 "batch_id": "batch_nan", "og": float('nan'), "fg": 1.010}]
        X, y_fg, y_time = prepare_features(data)
        self.assertEqual(len(X), 0)
    ...
    def test_train_models(self, mock_load):
        ...
        # Create mock data for 5 batches as list of dicts
        now = datetime.now(timezone.utc)
        all_data = []
        for b in range(5):
            for i in range(20):
        ...
        mock_load.return_value = all_data

        # Clean up existing models
        if os.path.exists(FG_MODEL_PATH):
            os.remove(FG_MODEL_PATH)
        if os.path.exists(TIME_MODEL_PATH):
            os.remove(TIME_MODEL_PATH)
