...
export function PeerComparisonWidget({ className }: PeerComparisonProps) {
    ...
    if (isLoading) {
        return (
            <div className={cn("bg-card border border-border rounded-xl p-6 h-full flex flex-col justify-center items-center gap-4 animate-pulse", className)}>
        ...
        <div className={cn("bg-card border border-border rounded-xl overflow-hidden flex flex-col h-full", className)}>
            <div className="p-4 border-b border-border flex items-center justify-between bg-secondary/20">
                <div className="flex items-center gap-2">
                    ...
                    <div>
                        <h3 className="text-sm font-medium text-foreground">Style Benchmarks</h3>
            ...
            <div className="p-5 flex-1 flex flex-col gap-4">
                <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-bold text-muted-foreground/80 uppercase tracking-widest">{data.primary_style} Averages</span>
                    <BarChart2 className="w-3 h-3 text-muted-foreground/60" />
                </div>
                ...
                <div className="mt-2 flex flex-col gap-2">
                    <span className="text-[10px] font-bold text-muted-foreground/80 uppercase tracking-widest">Recommendations</span>
...
function MetricBox({ icon, label, value }: { icon: React.ReactNode, label: string, value: string }) {
    return (
        <div className="bg-secondary/30 rounded-lg p-2.5 border border-border/30 flex flex-col gap-1">
            <div className="text-[10px] text-muted-foreground flex items-center gap-1.5 uppercase tracking-tighter">
                {icon} {label}
            </div>
